...
import { X, Send, RotateCcw, Loader2, CheckCircle2, XCircle, Lightbulb, Volume2, History } from "lucide-react"
import { Button } from "@/components/ui/button"
import type { CheckSentenceResponse } from "@/app/api/check-sentence/route"
...
export function SentenceComposerModal({
    ...
    const [result, setResult] = useState<CheckSentenceResponse | null>(null)
    ...
    const handleCheck = async () => {
        ...
        try {
            ...
            const data: CheckSentenceResponse = await res.json()
            setResult(data)
            ...
            setResult({ correct: false, feedback: "Không thể kết nối máy chủ.", checkedBy: 'heuristic' })
        } finally {
            setIsChecking(false)
        }
    }

    const handleRetry = () => { setResult(null); setSentence("") }
    ...
    // Does this provider give detailed AI feedback?
    const isAIProvider = result && !['heuristic'].includes(result.checkedBy)
    const isGoogleOnly = result?.checkedBy === 'google'
                ...
                >
                    ...
                    >
                        ...
                        <div className="flex items-center justify-between px-6 pt-6 pb-4 border-b border-border/60">
                            ...
                            <button onClick={handleClose} className="p-2 rounded-xl hover:bg-muted/80 text-muted-foreground hover:text-foreground transition-colors">
                                <X className="h-5 w-5" />
                            </button>
                        </div>
                        ...
                        <div className="px-6 py-5 overflow-y-auto space-y-5" style={{ maxHeight: "calc(92dvh - 80px)" }}>
                            ...
                            <div className="rounded-2xl bg-primary/5 border border-primary/20 p-4 flex items-center gap-4">
                                ...
                                <button onClick={() => speakText(hanzi)} className="p-3 rounded-xl bg-primary/10 hover:bg-primary/20 text-primary transition-colors flex-shrink-0" title="Nghe phát âm">
                                    ...
                                    <motion.div key="input" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-4">
                                        <div className="space-y-2">
                                            ...
                                            <textarea
                                                ...
                                                spellCheck={false}
                                            />
                                            <p className="text-xs text-muted-foreground text-right">Ctrl+Enter để kiểm tra • {sentence.length} ký tự</p>
                                        </div>
                                        <Button size="lg" className="w-full h-14 text-base font-semibold rounded-xl" onClick={handleCheck} disabled={!sentence.trim() || isChecking}>
                                            {isChecking ? <><Loader2 className="mr-2 h-5 w-5 animate-spin" />Đang kiểm tra...</> : <><Send className="mr-2 h-5 w-5" />Kiểm tra câu</>}
                                        </Button>
                                    </motion.div>
                                ) : (
                                    <motion.div key="result" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} transition={{ duration: 0.3 }} className="space-y-4">
                                        {/* Score Banner */}
                                        <div className={`rounded-2xl p-5 border-2 ${result.correct ? 'bg-emerald-50/60 dark:bg-emerald-950/20 border-emerald-500/40' : 'bg-destructive/5 border-destructive/30'}`}>
                                                ...
                                                <div className="flex-1">
                                                    <div className="flex items-center gap-2">
                                                        ...
                                                        <span className="text-xs bg-muted text-muted-foreground px-2 py-0.5 rounded-full">
                                                            {PROVIDER_LABELS[result.checkedBy] || result.checkedBy}
                                                        </span>
                                                    </div>
                                                    {/* Score bar — only for AI providers (not google/heuristic) */}
                                                    {result.score && !isGoogleOnly && (
                                                        <div className="flex items-center gap-2 mt-1.5">
                                                            <div className="flex-1 h-2 bg-muted rounded-full overflow-hidden">
                                                                <div className={`h-full rounded-full bg-gradient-to-r ${scoreGradient(result.score)} transition-all duration-700`} style={{ width: `${result.score * 10}%` }} />
                                                            </div>
                                                            <span className="text-sm font-bold tabular-nums">{result.score}/10</span>
                                                        </div>
                                                    )}
                                                </div>
                                            </div>
                                            ...
                                            <div className="mb-3">
                                                ...
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <button onClick={() => speakText(sentence)} className="p-2 rounded-lg hover:bg-muted text-muted-foreground"><Volume2 className="h-4 w-4" /></button>
                                                </div>
                                                {result.sentencePinyin && <p className="text-sm text-muted-foreground italic mt-0.5">{result.sentencePinyin}</p>}
                                            </div>
                                        ...
                                        {/* Correction — shown for AI providers when wrong */}
                                        {result.correction && !isGoogleOnly && (
                                            <div className="rounded-2xl bg-blue-50/60 dark:bg-blue-950/20 border border-blue-300/40 p-4">
                                                ...
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <button onClick={() => speakText(result.correction!)} className="p-2 rounded-lg hover:bg-blue-100 dark:hover:bg-blue-900/30 text-blue-500"><Volume2 className="h-4 w-4" /></button>
                                                </div>
                                            </div>
                                        )}
                                        ...
                                        {isGoogleOnly && (
                                            <p className="text-xs text-center text-muted-foreground">
                                                Google Dịch chỉ xác nhận câu hợp lệ, không phân tích ngữ pháp chi tiết.
                                            </p>
                                        )}

                                        {/* Actions */}
                                        <div className="flex gap-3 pt-1">
                                            <Button variant="outline" size="lg" className="flex-1 h-12 rounded-xl" onClick={handleRetry}>
                                                <RotateCcw className="mr-2 h-4 w-4" />Thử lại
                                            </Button>
                                            <Button size="lg" className="flex-1 h-12 rounded-xl" onClick={handleClose}>
                                                Tiếp tục học
                                            </Button>
                                        </div>
                                    </motion.div>
